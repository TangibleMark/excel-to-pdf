...
class ExcelReader:
    def __init__(self, excel_workbook, pdf):
    ...
    def get_rows(self, min_row=2, max_row=-1):
        ...
        for index, row in enumerate(row_range):
            if row[1].value == '#N/A' or None:
                ...
                pdf_filler.write_courses_taught(row[6].value)
                ...
                pdf_filler.write_date_of_absence(str(row[5].value).split(" ")[0])
                pdf_filler.write_time_from(str(row[7].value).split("-")[0])
                pdf_filler.write_time_to(str(row[7].value).split("-")[1])
                pdf_filler.write_hours_worked(row[8].value)
                pdf_filler.write_all_changes()
